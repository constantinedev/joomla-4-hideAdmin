...
class plgSystemAdminExile extends JPlugin {
	...
	public function __construct(&$subject, $config = array()) {
		...
		if (
			($this->_app->isClient('administrator') && !JFactory::getUser()->guest) || // not logged into the backend
			$this->_app->isClient('site') // accessing frontend
		)
		{
			$this->_pass = true;
		}
	...
	public function onAfterInitialise() {
		if ($this->_app->isClient('site') && \JFactory::getUser()->guest && $this->params->get('frontrestrict', 0))
		{
			return $this->_frontrestrict();
		}

		if (
			$this->_app->isClient('site') ||
			
			($this->_app->isClient('administrator') && !\JFactory::getUser()->guest))
		{
			return;
		}
	...
	private function _faillog($log) {
		if ($this->params->get('faillog', 0) && !$this->_logged)
		{
			$this->_logged = true;
			error_log($log);
		}
